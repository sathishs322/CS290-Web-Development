...
router.get("/photos/:id.jpg",  async function (req, res, next) {
...
router.get("/photos/:id.png",  async function (req, res, next) {
    const result= await getImageInfoById(req.params.id)
    getImageDownloadStreamByFilename(result.filename)
        .on("error", function (err) {
            if (err.code === "ENOENT") {
...
router.get("/thumbs/:id.jpg",  async function (req, res, next) {
